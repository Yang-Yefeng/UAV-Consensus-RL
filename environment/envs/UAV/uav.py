...
class UAV:
	...
	def reset_uav(self):
		[self.x, self.y, self.z] = self.param.pos0[:]
		[self.vx, self.vy, self.vz] = self.param.vel0[:]
		[self.phi, self.theta, self.psi] = self.param.angle0[:]
		[self.p, self.q, self.r] = self.param.pqr0[:]
		self.n = 0
		self.time = 0.
		self.throttle = self.m * self.g  # 油门
		self.torque = np.array([0., 0., 0.]).astype(float)  # 转矩
